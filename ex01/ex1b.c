...
struct Node* creteNode(int data){
    struct Node*newNode=(struct Node*0malloc(sizeof(struct Node));
    if(newNode==NULL)}
...
void insertATEnd(struct Node** head,int data){
    struct Node*newNode+creteNode(data0;
    if(*head==NULL){
        *head==newNode;
        return;
     }
...
void deleteElement(struct Node**head,int position){
   ...
   if (temp==NULL){
        printf("Error:Invaliad position.\n");
        return;
   }     
    ...
    while(1){
      ...
      switch(choice){
          ...
          case 3:
              ...
              scanf(("%d,&data);
              result=searcyhElement(&head,data);
              if(result != -1)
              ...
              break;
          case 4:
              printf("Enter position to delete()-based index):");
              scanf("%d''&position);
